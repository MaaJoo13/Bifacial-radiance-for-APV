...
for i in range(3):
    ...
    panelfront.append(np.mean(analysis.Wm2Front))
    panelback.append(np.mean(analysis.Wm2Back))

    analysis.analysis(oct, sim_name + "_groundscan_" + str(step), groundfrontscan, groundbackscan)
    groundfront.append(np.mean(analysis.Wm2Front))
    groundback.append(np.mean(analysis.Wm2Back))
